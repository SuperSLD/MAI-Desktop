...
QNetworkRequest ApiService::createRequest(QString url) {
    if (debug) qDebug() << "AppNetworkService: make request" << Qt::endl;
    if (debug) qDebug() << "AppNetworkService: " << SERVER_URL + "/" + url << Qt::endl;
...
QString ApiService::createResponseHandler(void (*handler)(QJsonObject, AppNetRepository*)) {
    ...
    if (debug) qDebug() << "AppNetworkService: handler UUID -" << handlerData.uuid << Qt::endl;
...
void ApiService::get(QString url, void (*handler)(QJsonObject, AppNetRepository*)) {
    ...
    QNetworkReply* reply;
    if (debug) qDebug() << "AppNetworkService: GET" << uuid << Qt::endl;
...
void ApiService::post(QString url, void (*handler)(QJsonObject, AppNetRepository*),  QJsonObject param) {
    ...
    QString uuid = createResponseHandler(handler);
    QNetworkReply* reply;
    if (debug) qDebug() << "AppNetworkService: POST" << uuid << Qt::endl;
...
void ApiService::onHttpResult(QNetworkReply *reply) {
    ...
    if (debug) qDebug() << "AppNetworkService: response UUID -" << uuid << Qt::endl;
    HandlerData handlerData;
    int handlerIndex = -1;
    QVariant statusCode = reply->attribute( QNetworkRequest::HttpStatusCodeAttribute );
    if ( statusCode.isValid() ) {
        if (debug) qDebug() << "AppNetworkService: STATUS CODE -" << statusCode.toString() << uuid  << Qt::endl;
    }
    for (int i = 0; i < handlers.size(); i++) {
        if (handlers[i].uuid == uuid) {
            handlerData = handlers[i];
            handlerIndex = i;
            if (debug) qDebug() << "AppNetworkService: success found handler -" << uuid << Qt::endl;
        }
    }

    if(!reply->error()) {
        QByteArray resp = reply->readAll();
        if (debug) qDebug() << "AppNetworkService: " << resp << uuid  << Qt::endl;
        QJsonDocument doc = QJsonDocument::fromJson(resp);
        QJsonObject obj;
        if(!doc.isNull()) {
            if(doc.isObject()) {
                obj = doc.object();
                if (debug) qDebug() << "AppNetworkService: success parse -" << uuid << Qt::endl;
                handlerData.handler(obj, rep);
            } else {
               if (debug)  qDebug() << "AppNetworkService: Document is not an object" << uuid << Qt::endl;
            }
        } else {
            if (debug) qDebug() << "AppNetworkService: Invalid JSON...\n" << uuid << Qt::endl;
        }
    } else {
        if (debug) qDebug() << "AppNetworkService: http response error -" << uuid << Qt::endl;
    }

    // удаляем хэндлер запроса если он найден
    if (handlerIndex >= 0) {
        handlers.removeAt(handlerIndex);
        if (debug) qDebug() << "AppNetworkService: handler remove -" << handlers.size() << Qt::endl;
    }
}
